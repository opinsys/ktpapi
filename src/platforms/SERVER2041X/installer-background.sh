...
sudo cat "$background_current_file" | awk '{ sub(/#385e77/,"#38775e"); print}' > "$background_current_file"
sudo cat "$background_versiontag_file" | awk '{ sub(/@VERSION/,"@VERSION - AUTOMAATTI"); print }' | awk '{ sub(/#385e77/,"#38775e"); print}' > "$background_versiontag_file"
